...
app.get('/payment/:memberID/:paymentID', fetchPayment, (req, res) => {
	if(req.query.success) {
		return res.render('payment-infos', {
			...
			msg: 'thank you, ',
			text: 'We have received your payment, thank you for your trust!',
		});
	}
	if(req.query.error) {
		const errors = {
			'already_paid': 'You have already made this payment!',
...
app.get('/check', (req, res) => {
	...
	paypal.detail(req.query.token, req.query.PayerID, async (err, data, invoiceNumber, price, paramArray) => {
		...
		const embed = new Discord.MessageEmbed()
			.setAuthor(`Thank you, ${user.username}`, user.displayAvatarURL())
			.setDescription('We have received your payment, we thank you for your trust!')
			.setColor('#0091fc');
		user.send(embed);
